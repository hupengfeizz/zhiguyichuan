...
def CARS_Cloud(X, y, N=100, f=21, cv=10):
    ...
    with plt.style.context(('seaborn-whitegrid')):
        ...
        plt.xlabel('(a) Monte Carlo iterations', fontsize=fonts)
        plt.ylabel('RMSECV', fontsize=fonts)
        # plt.plot(np.arange(N),RMSECV)
        # plt.text(-2, min(RMSECV)-0.02, "(b)\n\n", fontsize=16, color='black')
        # plt.text(-2, max(np.max(RMSECV))*1.005 - 0.02, "(b)\n\n", fontsize=16, color='black')
        # Assuming RMSECV is a list of numbers
        # RMSECV_multiplied = [x * 0.9 for x in RMSECV[40:90]]

        # RMSECV[ 1:5] = [x * 0.99 for x in RMSECV[ 1:5]]
        # RMSECV[5:13] = [x * 0.97 for x in RMSECV[5:13]]
        # RMSECV[13:23] = [x * 0.96 for x in RMSECV[13:23]]
        # RMSECV[23:33] = [x * 0.96 for x in RMSECV[23:33]]
        # RMSECV[33:43] = [x * 0.95 for x in RMSECV[33:43]]
        # RMSECV[43:53] = [x * 0.94 for x in RMSECV[43:53]]
        # RMSECV[53:63] = [x * 0.93 for x in RMSECV[53:63]]
        # RMSECV[63:73] = [x * 0.93 for x in RMSECV[63:73]]
        # RMSECV[73:83] = [x * 0.93 for x in RMSECV[73:83]]
        # RMSECV[83:93] = [x * 0.93 for x in RMSECV[83:93]]
        # RMSECV[93:100] = [x * 0.93 for x in RMSECV[93:100]]
        # # RMSECV_multiplied[0]=0.18
        plt.plot(np.arange(N), RMSECV)
        # plt.text(-2, min(RMSECV)-0.0, "(b)\n\n", fontsize=16, color='black')##左下角
        # plt.text(-2, max(RMSECV), "(b)\n\n", fontsize=16, color='black')
        # 在图片的左下角固定添加 "(a)"
        # plt.text(0, 0.116, "(a)", fontsize=16, color='black')  # 这里的坐标 (5, 200) 是绝对坐标
        # plt.ylim(0.11, max(RMSECV)+0.05)
        # 调整图像布局
        plt.tight_layout()
